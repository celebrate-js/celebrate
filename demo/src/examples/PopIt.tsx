...
export function PopIt() {
  return (
    <ExamplePageLayout
      icon="🫧"
      title="ポップイット"
      description="シリコンのプチプチ（Pop It）のように、タップするだけで可愛いエフェクトが見れるタイル集め。タイルを押すとそのテーマ専用のページ（暗い夜空・水面・雪景色…）に入り、そこでタップするたびに何度でも演出が見られる。💦🌸🎉⚡️🎆はカタログの既存variantをanchorで舞台に発火し、❄️🌀はカタログに無い独自の動き（ParticleField + 自作/既存のMotionProfile）をcelebrate(<SnowFall/>)のようにReactNodeとしてそのまま渡している。🥎だけは仕組みが違い、ミニゲーム（/examples/game）と同じ「出てくるターゲットをタップする」形式。"
    >
