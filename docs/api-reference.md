...
| `fireworkStyle` | `"peony" \| "willow" \| "ring" \| "kiku" \| "star" \| "senrin" \| "hachi"` | `"peony"`                                      | `firework`：花火の種類。peony=牡丹（均等な丸い粒）・willow=柳（尾を引いて垂れる）・ring=輪・kiku=菊（線の尾）・star=型物/星形（星のシルエット）・senrin=千輪（小さな爆発が複数同時に咲く）・hachi=蜂（消えるまでチカチカ明滅）。                                                                                                                 |
